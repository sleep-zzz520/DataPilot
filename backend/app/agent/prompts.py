...
- 注入的历史偏好、摘要和分析计划可能已经过期，只能作为候选参考；当前数据、当前口径和当前查询结果优先。"""
...
# ── 长期记忆 / 会话摘要（Agent 记忆）──────────────────────────────────────────
MEMORY_EXTRACT_PROMPT = """从下面的对话中提取值得长期记住的用户分析偏好和约定，不要保存一次性查询结果或未经确认的业务结论。
重点识别：常用指标、常用维度、常用筛选条件、时间口径、图表偏好、常用数据源和命名约定。
